...
class Adm extends CI_Controller
{
	...
	public function m_siswa()
	{
		...
		// $this->db->join('m_admin','m_admin.kon_id = m_siswa.id_siswa');
		$a['data'] = $this->db->get('m_siswa')->result();

		// $a['data'] = $this->db->query("SELECT m_siswa.*,
		// 							(SELECT COUNT(id_siswa) FROM m_admin WHERE level = 'siswa' AND kon_id = m_siswa.id_siswa) AS ada
		// 							FROM m_siswa")->result();


		if ($uri3 == "det") {
			$a = $this->db->query("SELECT * FROM m_siswa WHERE id_siswa = '$uri4'")->row();
			$this->j($a);
			exit();
		} else if ($uri3 == "simpan") {
			$ket 	= "";
			if ($p->id != 0) {
				$this->db->query("UPDATE m_siswa SET nama = '" . bersih($p, "nama") . "', nim = '" . bersih($p, "nim") . "', jurusan = '" . bersih($p, "jurusan") . "'	WHERE id_siswa = '" . bersih($p, "id") . "'");
				$ket = "edit";
			} else {
				$ket = "tambah";
			...
			$this->db->query("DELETE FROM m_siswa WHERE id_siswa = '" . $uri4 . "'");
			...
			$det_user = $this->db->query("SELECT id_siswa FROM m_siswa WHERE id_siswa = '$uri4'")->row();
	...
	public function h_ujian()
	{
		...
		if ($uri3 == "det") {
			...
			$a['hasil'] = $this->db->query("SELECT m_siswa.nama, tr_ikut_ujian.nilai, tr_ikut_ujian.jml_benar, tr_ikut_ujian.nilai_bobot
											FROM tr_ikut_ujian
											INNER JOIN m_siswa ON tr_ikut_ujian.id_user = m_siswa.id_siswa
	...
	public function ikut_ujian()
	{
		...
		$a['detil_user'] = $this->db->query("SELECT * FROM m_siswa WHERE id_siswa = '" . $a['sess_konid'] . "'")->row();
	...
	public function act_login()
	{
		...
		if ($j_data === 1) {
			...
			if ($a_data->level == "siswa") {
				$det_user = $this->db->query("SELECT nama FROM m_siswa WHERE id_siswa = '" . $a_data->kon_id . "'")->row();
				if (!empty($det_user)) {
